...
var InternalModuleFile = class extends InternalModule {
  ...
  generate_create_new() {
    return async (template, filename, open_new = false, folder) => {
      ...
      const new_file = await this.plugin.templater.create_new_note_from_template(
        template,
        folder,
        filename,
        open_new,
  ...
  generate_include() {
    return async (include_link) => {
      ...
      try {
        const parsed_content = await this.plugin.templater.parser.parse_commands(
          inc_file_content,
...
var Templater = class {
  ...
  async create_new_note_from_template(template, folder, filename, open_new_note = true) {
    ...
    const extension =
      template instanceof import_obsidian12.TFile ? template.extension || 'md' : 'md'
    const created_note = await errorWrapper(async () => {
      ...
      const path2 = app.vault.getAvailablePath(
        (0, import_obsidian12.normalizePath)(`${folderPath ?? ''}/${filename || 'Untitled'}`),
        extension,
      )
...
var EventHandler = class {
  ...
  update_trigger_file_on_creation() {
    if (this.settings.trigger_on_file_creation) {
      this.trigger_on_file_creation_event = this.plugin.app.vault.on('create', (file) =>
    ...
    functions: {
      ...
      include: {
        ...
        examples: [
          ...
          { name: 'File include section', example: '<% tp.file.include("[[MyFile#Section1]]") %>' },
...
var Editor2 = class {
  ...
  async setup() {
    ...
    this.templaterLanguage = import_state.Prec.high(
      import_language.StreamLanguage.define(window.CodeMirror.getMode({}, TEMPLATER_MODE_NAME)),
    )
    if (this.templaterLanguage === void 0) {
      log_error(
        new TemplaterError('Unable to enable syntax highlighting. Could not define language.'),
      )
    }
